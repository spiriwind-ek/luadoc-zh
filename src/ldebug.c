...
LUA_API int lua_getinfo (lua_State *L, const char *what, lua_Debug *ar) {
  ...
  if (*what == '>') {
    ci = NULL;
    func = s2v(L->top.p - 1);
    api_check(L, ttisfunction(func), "需要函数");
    what++;  /* skip the '>' */
...
static l_noret typeerror (lua_State *L, const TValue *o, const char *op,
  ...
  luaG_runerror(L, "尝试对 %s 值执行 %s%s", t, op, extra);
}
...
l_noret luaG_callerror (lua_State *L, const TValue *o) {
  ...
  typeerror(L, o, "调用", extra);
}


l_noret luaG_forerror (lua_State *L, const TValue *o, const char *what) {
  luaG_runerror(L, "无效的 'for' %s (number expected, got %s)",
...
l_noret luaG_tointerror (lua_State *L, const TValue *p1, const TValue *p2) {
  ...
  if (!luaV_tointegerns(p1, &temp, LUA_FLOORN2I))
    p2 = p1;
  luaG_runerror(L, "数值%s 没有整数表示", varinfo(L, p2));
}
...
l_noret luaG_ordererror (lua_State *L, const TValue *p1, const TValue *p2) {
  ...
  if (strcmp(t1, t2) == 0)
    ...
    luaG_runerror(L, "尝试比较 %s 和 %s", t1, t2);
}
...
l_noret luaG_errnnil (lua_State *L, LClosure *cl, int k) {
  ...
  luaG_runerror(L, "全局变量 '%s' 已定义", globalname);
}
