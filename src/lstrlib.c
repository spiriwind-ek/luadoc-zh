...
static void trymt (lua_State *L, const char *mtkey, const char *opname) {
  ...
  if (l_unlikely(lua_type(L, 2) == LUA_TSTRING ||
    ...
    luaL_error(L, "尝试用 '%s' %s 一个 '%s'", opname,
...
static const char *matchbalance (MatchState *ms, const char *s,
  ...
  if (l_unlikely(p >= ms->p_end - 1))
    luaL_error(ms->L, "格式错误的模式（'%%b' 缺少参数）");
  if (*s != *p) return NULL;
  else {
    int b = *p;
    int e = *(p+1);
...
static int add_value (MatchState *ms, luaL_Buffer *b, const char *s,
  ...
  if (!lua_toboolean(L, -1)) {  /* nil or false? */
    ...
    return luaL_error(L, "无效的替换值 (a %s)",
                         luaL_typename(L, -1));
  else {
      ...
      switch (*strfrmt++) {
        ...
        case 'q': {
          if (form[2] != '\0')  /* modifiers? */
            return luaL_error(L, "格式说明符 '%%q' 不能有修饰符");
          addliteral(L, &b, arg);
          break;
        }
        case 's': {
  ...
  if (size < SZINT) {  /* real size smaller than lua_Integer? */
    ...
    for (i = limit; i < size; i++) {
      if (l_unlikely((unsigned char)str[islittle ? i : size - 1 - i] != mask))
        luaL_error(L, "%d 字节整数不适合 Lua Integer", size);
    }
  }
