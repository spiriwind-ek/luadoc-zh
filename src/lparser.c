...
static l_noret error_expected (LexState *ls, int token) {
  luaX_syntaxerror(ls,
      luaO_pushfstring(ls->L, "需要 %s", luaX_token2str(ls, token)));
}
...
static l_noret errorlimit (FuncState *fs, int limit, const char *what) {
  ...
  msg = luaO_pushfstring(L, "%s 过多（限制为 %d） in %s",
                             what, limit, where);
  ...
  if (ls->t.token != ')') {  /* is 'parlist' not empty? */
    do {
      switch (ls->t.token) {
        ...
        default: luaX_syntaxerror(ls, "需要 <name> 或 '...'");
      }
...
static void breakstat (LexState *ls, int line) {
  ...
  luaX_syntaxerror(ls, "break 语句在循环外");
 ok:
...
static void forstat (LexState *ls, int line) {
  ...
  switch (ls->t.token) {
    ...
    default: luaX_syntaxerror(ls, "需要 '=' 或 'in'");
  }
