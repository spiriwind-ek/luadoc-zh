...
LUALIB_API int luaL_argerror (lua_State *L, int arg, const char *extramsg) {
  ...
  if (arg <= ar.extraargs)  /* error in an extra argument? */
    ...
    if (strcmp(ar.namewhat, "method") == 0) {  /* colon syntax? */
      ...
      if (arg == 0)  /* error in self argument? */
        return luaL_error(L, "在无效的 self 上调用 '%s' (%s)",
                               ar.name, extramsg);
  ...
  return luaL_error(L, "无效的 %s #%d 对 '%s' (%s)",
...
static size_t newbuffsize (luaL_Buffer *B, size_t sz) {
  ...
  if (l_unlikely(sz >= MAX_SIZE - B->n))
    return cast_sizet(luaL_error(B->L, "结果字符串过大"));
...
static int errfile (lua_State *L, const char *what, int fnameindex) {
  ...
  if (err != 0)
    lua_pushfstring(L, "无法 %s %s: %s", what, filename, strerror(err));
  else
...
LUALIB_API void luaL_checkversion_ (lua_State *L, lua_Number ver, size_t sz) {
  ...
  if (sz != LUAL_NUMSIZES)  /* check numeric types */
    luaL_error(L, "核心和库的数字类型不兼容");
  else if (v != ver)
    luaL_error(L, "版本不匹配：应用需要 %f，Lua 核心提供 %f",
